...
直觉思路：
- 回溯策略，数组中的每个元素，都有可能是target目标值的一部分，对于数组中，每个元素而言，有选中与不选中的可能
...
void traceback(const vector<int>& candidates, int target, int index)
{
    ...
    {
        traceback(candidates, target, index + 1);   // 不考虑当前的元素candidates[index]的情况

        // 考虑当前元素
        if (target - candidates[index] >= 0)
        {
            elem_of_result.push_back(candidates[index]);
            traceback(candidates, target - candidates[index], index);
