...
基类**将类型相关的函数**与**派生类不做改变直接继承的函数**区分对待，对于某些函数，**基类希望它的派生类各自定义成适合自身的版本**，此时基类就将这些函数声明为**虚函数**，可以将Quote类编写成：
...
- 派生类对象中的基类部分的protected成员对于**派生类以及友元的成员**来说是可访问的；(规定)
...
- new / delete是操作符，malloc / free是C语言中的库函数；

- new的执行过程：

  - 通过malloc分配**未初始化**的内存空间，返回空间的首地址，出现问题则抛出std::bad_alloc异常；
...
- delete的执行过程：
  ...
  - 通过free回收内存空间；
- 以上已经清晰展示了区别: 
  - new得到的是经过初始化的空间，malloc得到的是未初始化的空间；
  - delete包括了对对象的析构，而free则只释放空间；
...
这些特点确保了红黑树始终保持相对平衡，从而保证了树的整体性能。

红黑树的自平衡操作主要包括旋转和节点着色。通过这些操作，红黑树可以在插入和删除节点时保持树的平衡，从而确保树的高度保持在一个相对较小的范围内，使得搜索、插入和删除操作的时间复杂度保持在`O(log n)`。

需要注意的是，红黑树并不是严格意义上的平衡树，它的平衡性是相对的。但由于平衡性的要求较低，红黑树的插入和删除操作相对于其他平衡树来说更加高效，并且在实际应用中被广泛使用。
...
- 修饰局部变量，存储位置由栈->静态数据区，变化体现在生命周期的延长，但是作用域仍然不变；
- 修饰全局变量，全局变量`extern`既可以在本文件被访问到，也可以在同一个工程中其他源文件被访问，但是用static修饰过后则仅限于本文件可见；
- 修饰函数，使用情形与上述修饰全局变量类似，将范围缩小至仅本文件可见；
- 修饰类，修饰类需要分两种情况说明：
  - 如果是类中的函数用static修饰，那么表示该<font color=red>函数属于一个类而不是属于此类的任何特定对象</font>；
  - 如果对类中的某个变量使用static修饰，那么该变量在存储空间中仅有一个副本，通过类和对象去调用；
- 修饰类的使用细节：
  - 基于这么一个特点：`static`修饰的变量先于对象存在，因此：
  - 静态非常量数据成员，<font color=red>在要先在类外定义和初始化</font>，在类内仅是声明；
  - 静态成员函数则是在类外类内定义均可，不局限于位置；
  - 但是静态成员函数不存在this指针，this指针是指向本对象的指针，由于没有该指针，静态成员函数仅能访问类的静态成员；

**const作用: **

- 本身的作用很好理解，一个常量属性，值不允许改变；
- 修饰类的成员：
  - const成员变量只保证在某个对象生命周期是常量，而对于整个类而言是可以改变的；
    - 换句话说就是一旦创建了对象，则初始化之后是不允许再改变的；
  - const修饰类对象时，定义常量对象，那么常量对象只能调用常量成员函数，别的成员函数都不能调用；
- const成员函数中如果想要修改某个变量，可以用mutable进行修饰，这样即便在const函数中也能改变；

**C++中的const类成员函数**

- 对象调用成员函数时，会在形参列表的前面加一个形参this(隐式)，this指针指向的是当前对象的地址，显然，该地址是不能变的：`test* const;`
  - 当该成员函数的参数列表加了const：`void print() const;`，则此成员函数为常量成员函数，此时它的隐式this形参：`const test* const;`
  - 非常量对象可以调用类中的const成员函数，也可调用非const成员函数。

## 重载、重写、重定义

**重载: **英文名overload，指的是同一可访问区内被声明的几个具有不同参数列表的同名函数；

**重写: **英文名override，派生类中重新定义父类中除了函数体外完全相同的虚函数，要重写的函数不能是static的；

**重定义: **这个概念是针对非virtual函数，主要是根据作用域的特点而实现的，派生类也是自己的一个单独的类，会优先命中自身作用域内的同名函数；

## 类的几大构造函数

**无参构造函数: **即默认构造函数，如果程序员没有手动写无参构造，那么编译器自身会定义一个缺省的无参构造；

**一般构造函数: **可以有多种参数形式，程序员根据需要设定参数；

**拷贝构造函数: **拷贝构造函数的参数为对象本身的引用，用于根据一个已存在的对象复制出一个新的该类对象；

- 拷贝构造函数，体现拷贝同时又体现构造，为了避免一次到局部变量的拷贝，拷贝构造的参数一般用引用形式；
- 拷贝构造函数一般会与拷贝赋值运算符同时使用；

**类型转换构造函数: **本质上就是根据一个指定类型的对象创建一个本类的对象，在没有关键字explicit的情况下，注意隐式转换的可能；

**移动构造函数: **C++ 11标准引入的新型构造函数，通过将原对象移动到我们要构造的新对象上，减少拷贝所带来的时间损失；

- 如果类中定义了拷贝构造函数、拷贝赋值运算符或析构函数，编译器将不会自动生成移动构造函数。
- 移动赋值运算符常搭配移动赋值运算符使用；

## 四种类型转换

**`static_cast`: **用于基本类型之间的类型转换，没有动态类型检查，一般用来执行非多态的转换操作；

- 派生类到基类(向上造型)的转换是安全的，这个可以这么理解，派生类一定有基类的部分，但基类可不会一定有派生类的部分；
- 因此基类到派生类的转换不一定安全；
- 但是`static_cast`由于没有动态类型检查，因此不会管这部分的安全与不安全；
- 因此说一般执行非多态的操作；

**`dynamic_cast`: **用于在具有继承关系的指针或引用之间进行安全的向下转型-派生类到基类的转换。动态转换在运行时进行类型检查，如果转换不安全，则返回空指针(对于指针类型)或抛出std::bad_cast异常(对于引用类型)；

- type-id必须是类指针、类引用或者void*；
...
```c++
const int num = 10;
int* mutableNum = const_cast<int*>(&num);  // 去除常量性，使得指针可以修改num的值
const int* constPtr = const_cast<const int*>(mutableNum);  // 添加常量性，使得指针不能修改num的值
```

**`reinterpret_cast`: **用于将一个指针或引用转换为另一个类型的指针或引用，甚至不考虑类型之间的关系；

- 这一类转换通常是非常危险的，因为它不进行类型检查；

## 堆-栈的区分

**栈: **由编译器进行管理，在需要时由编译器自动分配空间，在不需要时自动回收空间，一般保存局部变量和函数参数等信息；

- 栈空间一般是连续的内存空间，在函数调用的时候首先入栈的是主函数的下一条可执行指令的地址(<font color=red>先标红</font>)；
- 在大多数编译器中，参数都是从右向左入栈，这样第一个参数就会在栈顶，这样的设计模式主要是考虑到可变参的函数，因为在调用可变参之前就需要解析各参数类型；
- <font color=red>这部分的详细流程值得再回顾</font>；

**堆: **由程序员管理，需要手动回收；

- 堆空间一般不连续，实际上系统中有一个空闲链表，当有程序申请的时候，系统遍历空闲链表找到第一个大于等于申请大小的空间分配给程序；
- 分配空间的时候，也会在空间头部写入内存大小，方便delete回收空间，如果存在剩余，剩余空间又会插入到空闲链表中，这是产生内存碎片的原因；
- 堆由低地址向高地址拓展；

## 深拷贝与浅拷贝

我们假设这么一个前提：通过等号将一个类赋值给另一个类；

- 正常调用拷贝构造函数，默认的拷贝构造是浅拷贝的，完成成员的一一复制；
- 但如果成员是指针，这个时候就出现问题了，两个类对象中的指针成员指向同一个地址；
- 当其中一个类执行了析构函数，另一个类尝试去删除一个不存在指针空间；
- 这样就导致了野指针问题；
- 此时就应该调用深拷贝，在堆内存中另外申请空间来存储数据，从而解决野指针问题；

## 四个常用关键字

**`const`**

- const定义的常量是变量带类型，如果定义在作用域内，**受作用域**范围限制；
- const在**编译**、**链接**过程中起作用，具有数据类型，会进行类型检查；
- 使用const关键字定义的常量在编译时会分配内存空间，它们通常存储在数据段(data segment)中；
- const不能重定义；

**`define`**

- define定义的常量则不带类型，如果定义在作用域内，**不受作用域**范围限制；
- const在**预处理**过程中起作用，只是简单的文本替换，不做类型检查；
- 使用define定义的常量在预处理阶段内存空间，存储在代码段(code segment)中；
- define可以重定义，但是要先通过#undef取消某个符号的定义以进行重定义；

**`typedef`**

- typedef为已有变量定义别名，如果定义在作用域内，**受作用域**范围限制；
- typedef在编译阶段有效，且带类型检查的功能；
  - 类型检查体现在，即便是定义的别名，比如int->intetgal，你也不能去输出一个字符串；
- typedef是一个指令，只在编译时起作用，它不占据内存空间(我的理解)；
- typedef可以重定义，但是不能重定义为已有的类型别名；

**`inline`**

- inline的作用是直接将函数代码插入到调用它的地方，而不是通过调用函数然后压栈的方式，可以提升性能；同函数一道，**受作用域**限制；
- inline在**编译**阶段进行替换，将函数调用替换为函数内容，具有类型检查；
  - 类型检查体现在对函数参数的检查，检查是否一致；
- inline是一个指令，在编译时起替换作用，它不占据内存空间(我的理解)；
- inline不能重定义，否则冲突；

## POSIX有关接口

**明确线程与进程:** 其实线程与进程本质上就是一个函数调用，这么理解是不对的：

- 进程更多的是一个完整的程序，进程中也许会包含多个函数，当程序遇到了fork之后，fork又会创建出一个子进程，子进程又是一个单独的程序，有自己的行为；
- 线程比较接近我们所说的函数，他是最基本的执行单元，每个线程都有自己的调用栈和执行上下文，可以并发地执行。

**一些系统调用**

`fork`: 一个用于创建新进程的系统调用。它在调用时会创建一个与原始进程几乎完全相同的子进程，包括代码、数据和打开的文件等。

`exec`: 用于在当前进程中执行其他程序。它可以用来替换当前进程的代码和数据，从而在当前进程的上下文中执行新的程序。

- 由于会替换当前的进程和数据，因此该函数一般于子进程中去执行；

`wait`: 一个系统调用，用于等待子进程的终止并获取其终止状态。

## 类的静态分配和动态分配

**概念上**

**静态分配(Static Allocation):** 在编译时为对象分配内存空间。静态分配的对象在程序的整个生命周期中都存在，它们的内存空间在编译时就固定下来了，不会在运行时动态改变。

- 静态分配的对象通常是在栈上分配的，例如局部变量和函数的参数。当程序运行到离开作用域的位置时，静态分配的对象会自动被销毁。

**动态分配(Dynamic Allocation):** 在运行时为对象分配内存空间。动态分配的对象的内存空间是在程序运行时动态分配的，并且可以在运行时动态改变。

- 动态分配的对象通常是在堆上分配的，通过使用new操作符来创建对象。动态分配的对象需要手动释放内存空间，否则会导致内存泄漏。释放内存空间的操作是通过delete操作符来完成的。
...
int main() {
    Example Object1; // 静态分配对象
    auto point_to_class = new Example();	// 动态分配对象

    // 在此处执行其他操作
	
    delete point_to_class;	// 删除这一片空间，需要我们手动处理
    return 0;
}

```

经过前面一系列概念，我们也可以看到，静态一般针对的是对象本身，动态一般针对的是指针或者引用；



# 宏的相关知识点

在C++中，宏(Macro)是一种预处理指令，用于**在编译之前**对代码进行替换和扩展。

宏定义了一组规则，指定了在代码中出现特定宏名称时要执行的替换操作。

## 注意事项

由于宏是简单的文本替换，它确实可以提高代码的可读性和可维护性，以及减少代码重复，但是要注意潜在问题：
...
- 宏的使用导致参数多次求值；
  ...
  int main() {
      int x = 10;
      int y = MAX(x, getRandomNumber());	// ((a) > (b) ? (a) : (b))中，可以看到b被执行两次。但取随机数，两者会不一样，这就是重复求值
...
- 宏不具备类型检查，可能导致类型相关的错误；
  ...
  int main() {
      int num = 5;
      double result = SQUARE(num + 1);	// 由于仅仅是文本替换，会变成(num + 1 * num + 1)，结果不符合我们的预期
...
- 在C语言中，文件描述符(file descriptor)通常是一个整数，用于标识打开的文件。
- 当我们打开一个文件时，操作系统会为该文件分配一个文件描述符，并将其返回给我们。

- **每个文件描述符都会关联一个缓冲区**，用于临时存储待写入或待读取的数据，缓冲区是一个内存区域，可以暂时保存数据，以便在适当的时候写入或读取。

- 当我们进行写入操作时，数据首先被写入到文件描述符关联的缓冲区中，而不是立即写入到文件中，缓冲区的目的是优化文件操作的性能，减少频繁的IO操作。

然而，并不是每次写入操作都会立即将数据从缓冲区写入到文件中，数据在缓冲区中可以累积一定量，或者等待特定的条件，例如缓冲区满了、程序结束时或者显式地调用`fflush()`函数的时候。

当我们调用`fflush()`函数时，它会强制将与文件描述符关联的缓冲区中的数据立即写入到文件中。这样，数据就不再停留在缓冲区中，而是被写入到了文件中。
