...
class Empty_class {
    ...
    * 然而类大小大小不是0，至少为1，因为至少会分配给他一个地址，那么就又衍生出了一个新的疑问：
    * - 在我的64位的机器上为啥也是1字节，不应该是8字节吗？
    *   - 这就需要考虑具体的编译器环境，由于内存对齐的原则存在，给1字节和给8字节相比，可以
    *   - 有空间容纳更多的东西，这么理解不知是否妥当； 
    */
    public:
...
class Derived : public Base {
    public:
        void func1() override { cout << "This is Derived::func1()!" << endl; }
        virtual ~Derived () {}
    private:
        int newdata;
        int newadd;
